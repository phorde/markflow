...
| 2026-04-21 | Codex | ci-tooling | Gate platform-specific runtime attributes behind `getattr` and preserve POSIX process-group timeout cleanup. | Linux CI `mypy` failed on Windows-only `subprocess.CREATE_NEW_PROCESS_GROUP` and `ctypes.windll`; local Windows `mypy` also needed a portable `signal.SIGKILL` fallback. | `scripts/run_with_timeout.py`, `markflow/pipeline.py`; `python -m mypy --platform linux markflow services scripts`; `python -m mypy markflow services scripts`; `python -m pytest tests\unit\test_run_with_timeout.py --no-cov`; GitHub run `24646622337` jobs `test (3.10/3.11/3.12)` | Restores CI matrix portability without weakening timeout behavior. | active |
| 2026-04-22 | Codex | deployment/render | Use Render Blueprint deployment with free web services for API, frontend, and worker plus free Key Value for Redis Streams. | Render's Free instance type is not available for background workers, so the initial zero-cost deployment preserves service isolation by running the worker container as a web service with a health endpoint. | `render.yaml`, `services/api/api.py`, `services/api/Dockerfile`, `services/frontend/Dockerfile`, `services/worker/entrypoint.py`, `tests/unit/test_web_foundation.py`; `python -m pytest tests/unit/test_web_foundation.py -q --no-cov`; `python scripts/check_service_boundaries.py`; `python -c "import pathlib, yaml; yaml.safe_load(pathlib.Path('render.yaml').read_text(encoding='utf-8')); print('render.yaml parsed')"`; `npm run build`; checkpoint: direct Codex Render deploy session 2026-04-22 | Enables a free demo deployment on `app.phorde.com.br` and `api.phorde.com.br` while keeping API, frontend, worker, and Redis boundaries explicit. | active |
