...
- **Global scope**: `/home/ubuntu/dev/markflow/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/ubuntu/dev/markflow/.claude/notes/`)
...
2. Glob `/home/ubuntu/dev/markflow/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/ubuntu/dev/markflow/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/ubuntu/dev/markflow/.claude/notes/` — works in any directory
