...
- **Global scope**: `C:/ExtratorLaudos/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ExtratorLaudos/.codex/notes/`)
...
2. Glob `C:/ExtratorLaudos/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/ExtratorLaudos/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ExtratorLaudos/.codex/notes/` — works in any directory
